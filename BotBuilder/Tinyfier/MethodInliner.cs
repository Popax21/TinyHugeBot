            ...
            foreach(CilInstruction inlineInstr in inlineBody.Instructions) {
                ...
                switch(inlineInstr!) {
                    ...
                    case {} when inlineInstr.OpCode == CilOpCodes.Ldarga || inlineInstr.OpCode == CilOpCodes.Ldarga_S: {
                        ...
                        if(instrs.Length == 1 && instrs[0].IsLdloc()) body.Instructions.Insert(idx++, CilOpCodes.Ldloca, instrs[0].GetLocalVariable(body.LocalVariables));
                        else if(instrs.Length == 1 && instrs[0].IsLdarg()) body.Instructions.Insert(idx++, CilOpCodes.Ldarga, instrs[0].GetParameter(body.Owner.Parameters));
                        else throw new InvalidCilInstructionException("Ldarga is not supported for non-trivial arguments in inlined methods");
                    } break;
